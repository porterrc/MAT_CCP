...
for i = 1:length(files)
    ...
    l = 1;
    f2 = waitbar(0,'Calculating RFs');
    for j= 1:length(events);
        waitbar(j/length(events),f2,['Processing Event ' num2str(j) ' of ' num2str(length(events)) ' for station ' s_name]);
        ...
        if (length({EQ_Data.(char(events(j))).station}) == 3)    
            ...
            if (any(tsz.data) & any(tsn.data) & any(tse.data));
                ...
                data = detrend(data);
                bpFilt = designfilt('bandpassiir','FilterOrder',20,'PassbandFrequency1',hp,'PassbandFrequency2',lp, 'SampleRate',rsr); % build  filter
    ...
    clear RPs baz RFs Err RFr RFt ErT
    end
end
close(f1);
load Stns.mat
