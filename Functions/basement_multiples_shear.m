function [thick] = basement_multiples_shear(rf_dist,xd,yd,zd,x_min,x_max,y_min,y_max,sv_model,pv_model);
...
a = size(topo);
slength = round(2/(rf_dist(2)-rf_dist(1))); % smooths to 2 km
...
% read in vel model to migrate time to depth
 dzi = zd(2)-zd(1);
 vp = flipud(pv_model); % interpolate vp and vs to depth
 vs = flipud(sv_model); % interpolate vp and vs to depth
 qa = sqrt(1./vp.^2 - p.^2);
 qb = sqrt(1./vs.^2 - p.^2);
...
plot(rf_dist,-smooth(d0,slength),'r_','LineWidth',2)
plot(rf_dist,-smooth(d1,slength),'r:','LineWidth',2)
plot(rf_dist,-smooth(d2,slength),'b:','LineWidth',2)
