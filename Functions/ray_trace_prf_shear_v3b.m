...
function [z, xpierce,ypierce, seisout]  = ray_trace(Data,sname,stnx,stny,stel,dzi,z_max,sx,sy,sz,svmodel,vmodel,minv,mxamp,mnamp,blim)
...
for i = 1:length(sname);
    ...
    if contains(fd_name,'_')
        ...
        fd_name = char(finfo(2));
    end
    if isfield(Data,fd_name);      
        p = Data.(fd_name).RFs_in_Time.RPs;
        backaz = Data.(fd_name).RFs_in_Time.BAZs;
        if ~isfield(Data.(fd_name).RFs_in_Time,'ID')
            continue
        end
        seis = Data.(fd_name).RFs_in_Time.ID.RFs;
        time = Data.(fd_name).RFs_in_Time.ID.Time;
    ...
    %    for j = find(Data.(char(sname(i))).RFs_in_Time.ID.Grade <= 1);
        for j = 1:size(Data.(fd_name).RFs_in_Time.ID.RFs,1);
