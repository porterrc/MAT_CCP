...
%files = ls('./Y*RF_Data.mat');
files = sort(strsplit(files,{'.mat','\t','\n'}));
files(1) = [];
rsr = 20; %resample rate
lp = 5; % lowpass
hp = .25; %highpass
Data = [];
for i = 1:length(files)
    baz = []; RPs = []; RFr = []; Err = []; RFt = []; Ert = []; Dsnr = []; data = []; s_name = [];
    ...
    evs = fields(EQ_Data);
    s_name = EQ_Data.(char(evs(1)))(1).station;
        ev = 1;
        tname = [];
        while isempty(s_name)
            if ev == length(evs);
               tname = strsplit(char(files(i)),{'/','_RF_Data'});
               tname = char(tname(end-1))
               s_name = tname;
            end
            ev = ev + 1;
    ...
    disp([s_name ' ' char(datetime('now')) ' ' num2str(length(events)) ' events']);
    l = 1;
    for j= 1:length(events);
        if mod(j,100) == 0;
             disp([s_name ' ' char(datetime('now')) ' ' num2str(j) ' completed']);
        end
        ...
        if (length({EQ_Data.(char(events(j))).station}) == 3)    
            ...
            %%cut data
            tsz = getsampleusingtime(tsz,stimes(1)+(50-40),stimes(1)+(50+100));
            tse = getsampleusingtime(tse,stimes(1)+(50-40),stimes(1)+(50+100));
            tsn = getsampleusingtime(tsn,stimes(1)+(50-40),stimes(1)+(50+100));
            if (any(tsz.data) & any(tsn.data) & any(tse.data));
                ...
                data(:,4) = r;
                data(:,5) = t;
                if length(data) <= 60*rsr;
                    data = [];
                    continue
                end
                namp = mean(abs(data(20.*rsr:40.*rsr,:)));
                namp = namp([1 4 5]);
                samp = mean(abs(data(40.*rsr:60.*rsr,:)));
                samp = samp([1 4 5]);
                snr = samp./namp;
                data(1:30*rsr,:) = [];
                L = size(data,1); % Build Taper
                ...
                if size(data,1) >= 2001;
                    ...
                    [itr rvar] = makeRFitdecon(data(:,4),data(:,1),1/rsr,0,40,10,2.5,1200,0.00001,0);
                    [itt tvar] = makeRFitdecon(data(:,5),data(:,1),1/rsr,0,40,10,2.5,1200,0.00001,0);
                    RFr(l,:) = itr; 
                    Err(:,l) = rvar(end);
                    RFt(l,:) = itt; 
                    Ert(:,l) = tvar(end);
                    ...
                    ttime(1:30*rsr,:) = [];
                    ttime = ttime-ttime(1);
                    [Eitr] = ETMTM_new(data(:,1)',data(:,4)',ttime,4,7,-10,40,'Ps');
                    [Eitt] = ETMTM_new(data(:,1)',data(:,5)',ttime,4,7,-10,40,'Ps');
                    ERFr(l,:) = Eitr(1:length(-10:1/rsr:40-1/rsr));
                    ERFt(l,:) = Eitt(1:length(-10:1/rsr:40-1/rsr));
                    l = l + 1;
                end
            end
        end
    ...
    if ~isempty(RPs) 
    s_name = [EQ_Data.(char(evs(1)))(1).network '_' s_name];
    if isstrprop(s_name(1),'digit')
        s_name = ['N' s_name];
    end
    ...
    Data.(s_name).RFs_in_Time.ID.Time = [-10:1./rsr:40];
    Data.(s_name).RFs_in_Time.ETMTM.RFs = ERFr;
    Data.(s_name).RFs_in_Time.ETMTM.RFts = ERFt;
    Data.(s_name).RFs_in_Time.ETMTM.Time = -10:1/rsr:40-1/rsr;
    Data.(s_name).RFs_in_Time.SNR = Dsnr;
    baz = []; RPs = []; RFr = []; Err = []; RFt = []; Ert = []; Dsnr = []; ERFr = []; ERFt = [];
    end
end
load Stns.mat
save('RF_2.5_mat.mat','Data','Stns')